...
int main (int argc, char** argv)
{
    ...
    const char *request1 = "ps";
    write (TO_KERNEL, request1, strlen (request1));

    printf ("trapping to %d in pid %d\n", ppid, pid);
    assert(kill (ppid, SIGTRAP) == 0);
    ...
    printf ("writing in pid %d\n", pid);
    const char *request2 = "systime";
    write(TO_KERNEL, request2, strlen (request2));
